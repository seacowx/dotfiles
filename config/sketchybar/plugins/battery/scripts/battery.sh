...
render_bar_item() {
	...
	if [[ ${CHARGING} != "" ]]; then
		...
		8[0-9]) ICON="󰂋" COLOR="$GREEN" ;;
		7[0-9]) ICON="󰂊" COLOR="$GREEN" ;;
		6[0-9]) ICON="󰂊" COLOR="$YELLOW" ;;
		5[0-9]) ICON="󰂉" COLOR="$YELLOW" ;;
		4[0-9]) ICON="󰂉" COLOR="$PEACH" ;;
		3[0-9]) ICON="󰂈" COLOR="$PEACH" ;;
		2[0-9]) ICON="󰂇" COLOR="$RED" ;;
